...
trade_results = {}
trade_results['action_dict'] = action_dict
...
for i in indices:
    ...
    if (trade_record is not None) and (len(trade_record) > 0):
        ...
        trade_results[str(i)+"_beta"] = EG_Test.beta
        trade_results[str(i)] = trade_record


with open('trade_results_'+ x_stock + '_' + y_stock +'.pickle', 'wb') as handle:
    pickle.dump(trade_results, handle, protocol=pickle.HIGHEST_PROTOCOL)
